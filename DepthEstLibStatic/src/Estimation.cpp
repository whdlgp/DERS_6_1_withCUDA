...
#else
        for(int i=0; i<m_iNumOfLabels; i++)  // 왜 한꺼번에 할당을 안받죠? 
        {
            if(errors[i]!=NULL) 
            {
                free(errors[i]);
...
bool CEstimation::xInit(CParameterDepthEstimation cParameter)
{
	...
	// 왜 순서가 Left-Right-Centor? 그리고 명칭이 왜 Virtual?
	// getDEmode == 3이면 depth 카메라?
...
#else
    for(i=0, pos=m_iPicsize; i<m_iNumOfLabels; i++, pos+=m_iPicsize) //왜 한꺼번에 할당을 안할까요?
    {
...
bool CEstimation::setup_from_search_range(int iMinSearchRange, int iMaxSearchRange, int iMinDisparityRange, int iMaxDisparityRange, CvMat *matIn[3], CvMat *matEx_c2w[3], unsigned int uiDepthType, unsigned int uiBaselineBasis, unsigned int uiMatchingMethod)
{
	...
	// 255 보다 큰 MaxDisparity 인 경우 이렇게 표시 하는 것으로 보임.
    ...
    m_iNumOfLabels = iMaxSearchRange - iMinSearchRange + 1;  // 검색할 disparity 수 (subpixel 고려)
...
//
// disparity Range 와 baseline 카메라 focal length로 부터 Znear/far 계산 
...
void CEstimation::caloc_z_range(double focal_length, double baseline, double disparity_offset, CvMat* matEx_c2w, int iMinDisparity, int iMaxDisparity, unsigned int uiDepthType)
{
    ...
    // m_dZfar/baseline =  focal_length/minDisparity   // 대부분 0 
    ...
    if(uiDepthType)
    {
        m_dZnear = cvmGet(matEx_c2w, 2, 2) * m_dZnear + cvmGet(matEx_c2w, 2, 3);  // 보통 0 아닌가요?
...
// Lookup 태이블 생성 
// m_aiLabel2Disparity[left/right][매칭에의한 disparity] = 실제 disparity
//
...
//
// Wrapping/Agent 함수. 실제 함수는 function pointer로 연결됨.
//
...
//
// Pixel 하나를 비교 함. BM 함수 중에 가장 쉬운 함수이므로, 여기서부터 분석 하는것이 바람직  
...
void CEstimation::block_matching_disparity_pixel(CIYuv<ImageType> *yuvLeft, CIYuv<ImageType> *yuvRight, CIYuv<ImageType> *yuvCenter, BYTE ***srcSEGM)
{
    ...
    for(j=pp=0; j<m_iHeight; j++)
    {
        for(i=0; i<m_iWidth; i++, pp++) // error[depth][position of pixel], GC을 위하여 이렇게 구성.  
        {
            for(d=0; d<m_iNumOfLabels; d++)  // disparity search 범위네에서 일단 모두 계산  
            {
#ifdef POZNAN_TWOVIEW_SUPPORT
                if(yuvLeft)
                {
#endif
#ifdef SUB_PEL_PRECISION
                target_pixel_u = i*m_iPrecision + m_aiLabel2Disparity[0][d]; // 정밀도와 index to disparity maaping table 
#else
                target_pixel_u = i + m_aiLabel2Disparity[0][d];
#endif
                if(target_pixel_u>=0 && target_pixel_u<m_iMaxWidth)
                    error_l = byte_abs[yuvLeft->Y[j][target_pixel_u] - yuvCenter->Y[j][i]];  // ABS-DIFF 계산 (태이블사용)
                else
                    error_l = 255;  // MAX 값 (즉 선택을 안하려고함)
#ifdef POZNAN_TWOVIEW_SUPPORT
                }
                else error_l = 255;
#endif
...
//
// Block Matching 알고리즘 구현 (conventional 과 disparity based의 모두 사용)
// LEFT-CENTER-RIGHT YUV 420 입력 
// Segmentation 정보는 사용하지 않음 ...
...
// NICT 3x3 block matching start
  ...
  if ( BlockSize !=0 )
  {
    ...
    j = 0, cj = 0, j1= j + 1;
        // i=0
        i = 0, ci = 0, i1 = 1;
            for(d=0; d<m_iNumOfLabels; d++) // 왠 Labels?
            {
#ifdef POZNAN_TWOVIEW_SUPPORT
                if(yuvLeft)
                {
                ...
                u1 = target_pixel_u + 1;
#endif
                if(target_pixel_u>=0 && u1<m_iMaxWidth)  // 왼쪽과 비교 
                {
                    cu = target_pixel_u >>1;
                ...
                u1 = target_pixel_u + 1;
#endif
                if(target_pixel_u>=0 && u1<m_iMaxWidth)  // 오른쪽과 비교 
                {
                    cu = target_pixel_u >>1;
...
//-----------------------------------------------------------------------------
// Reference로 부터 Depth estimation error 를 계산함. 
// 여러가기 형태로 Reference depth가 주어짐
//
...
double CEstimation::data_term_disparity(int label, int pp)
{
	...
	//* //Occ Gdy Occ jest zsyntezowan?map?g쿮bi
	...
	/* //Occ Gdy Occ jest zsyntezowan?map?g쿮bi i odejmowanie kosztu
	int x = pp % m_iWidth;
	int xl  = pp % m_iWidth + m_aiLabel2Disparity[0][label]/m_iPrecision;
	int xr  = pp % m_iWidth - m_aiLabel2Disparity[1][label]/m_iPrecision;
	int ppl = pp +  m_aiLabel2Disparity[0][label]/m_iPrecision;
...
void CEstimation::center_image_segmentation(BYTE ***srcSEGM)
{
    ...
    // Read Center Image  (OpenCV 메모리로 복사)
    ...
    dst_img_segm = cvCloneImage (src_img_segm);  // 왜 미리 복사?
    clusters     = cvCreateMat  (m_iWidth * m_iHeight, 1, CV_32SC1);  // 해당 크러스트 id?
    ...
    switch (m_iSegmentationMethod){
		...
		// @TODO OpenCV 가 1차원으로 데이터 입력을 받다 보니 생기는 불상사
    ...
    cvSaveImage("LowColorImage.bmp", dst_img_segm);  // segmentation 결과를 이미지로 저장?
	printf("The segmentation is done.\n");

	// 여기서 부터는 Labeling 하는 것으로 보니는데. 왜 이걸 직접 이렇게 구현 했을까?
	// OpenCV 에서 내부적으로 이미 했을 꺼 같은데.
...
void CEstimation::estimation_occ()
{
  ...
  for(y=0,pp=0; y<m_iHeight; y++)   
  {
    ...
    {
      ImgDisparity->imageData[y*ImgDisparity->widthStep+x] = m_aiLabel2Disparity[0][labels[pp]]; //Powinien by?drugi taki blok dla drugiego obrazu (mo풽 po porstu odwo퀉wa?si?do labels a nie do imgdisp
    }
  }
