...
public:
  ...
  bool			setVerticalUpsampleFilter		(unsigned int filter, unsigned int precision);
  bool			upsamplingVertical					(CIYuv *src);

	bool			upsampling					(CIYuv *src);

	int				getSmapling				()	{ return sampling; }  // mis-spelling kkk
...
private:
  ...
  void (*FuncFilterVertical)(PixelType **in, PixelType **out, int width, int height, int padding_size);
...
bool CIYuv<PixelType>::readOneFrame(FILE *fp, int frameno)
{
  long offs = long(size_in_byte)*long(frameno);
	if(Y==NULL || fp==NULL) return false;
...
template <class PixelType>
bool CIYuv<PixelType>::setVerticalUpsampleFilter(unsigned int filter, unsigned int precision)
{
	switch(precision)
	{
	case 1: // INTEGER PEL
		FuncFilterVertical = DummyFilter_2D;
		break;
	case 2: // HALF PEL
		switch(filter)
		{
		case 0: // BI-LINEAR
			FuncFilterVertical = VerticalLinearFilter_2D_half;
			break;
		case 1: // BI-CUBIC
			FuncFilterVertical = VerticalCubicFilter_2D_half;
			break;
		case 2: // AVC
			FuncFilterVertical = VerticalAVCFilter_2D_half;
			break;
		default:
			return false;
			break;
	...
	case 4: // QUARTER PEL
		...
		{
		case 0: // BI-LINEAR
			FuncFilterVertical = VerticalLinearFilter_2D_qpel;
			break;
		case 1: // BI-CUBIC
			FuncFilterVertical = VerticalCubicFilter_2D_qpel;
			break;
		case 2: // AVC
			FuncFilterVertical = VerticalAVCFilter_2D_qpel;
			break;
		default:
			return false;
			break;
...
template <class PixelType>
bool CIYuv<PixelType>::upsampling(CIYuv *src)
{
	// need to check buffer size

	FuncFilter(src->Y, Y, src->getWidth(), src->getHeight(), 0);
	FuncFilter(src->U, U, src->getWidthUV(), src->getHeightUV(), 0);
	FuncFilter(src->V, V, src->getWidthUV(), src->getHeightUV(), 0);

	return true;
}

template <class PixelType>
bool CIYuv<PixelType>::upsamplingVertical(CIYuv *src)
{
	// need to check buffer size

	FuncFilterVertical(src->Y, Y, src->getWidth(), src->getHeight(), 0);
	FuncFilterVertical(src->U, U, src->getWidthUV(), src->getHeightUV(), 0);
	FuncFilterVertical(src->V, V, src->getWidthUV(), src->getHeightUV(), 0);
